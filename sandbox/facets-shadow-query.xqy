declare namespace log = "local";
declare function log:facet-frequencies($facets as map:map, $zero as xs:integer?) as map:map {
...
declare function log:facet-frequencies($facets as map:map) as map:map {
  log:facet-frequencies($facets, ())
};
...
(::
 ...
 :)
declare function log:shadow-values(
  ...
  let $all as map:map := log:facet-frequencies(cts:values($range-indexes, $start, ("map"), $shadow-query), 0)
  let $some as map:map := log:facet-frequencies(cts:values($range-indexes, $start, ($options, "map"), $query))
...
declare function log:shadow-values(
  ...
  log:shadow-values($range-indexes, $start, $options, $query, $shadow-query, $quality-weight, ())
};
declare function log:shadow-values(
  $range-indexes as cts:reference*, 
  $start as xs:anyAtomicType?, 
  ...
  log:shadow-values($range-indexes, $start, $options, $query, $shadow-query, (), ())
};
...
return 
  xdmp:to-json(
    log:shadow-values(cts:json-property-reference("sender"), (), (), $query, $shadow-query)
  )
